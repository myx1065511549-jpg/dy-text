...
VOC 关键词按真实弹幕优化一版并写进看板。命中率 **27% -> 70%**(4849条真实带货弹幕实测)。

方法:先剔除机器人刷屏和纯表情,只看真实观众弹幕,统计未命中高频句,按观众真实说法配词(不用书面词)。

实测暴露的盲区与修法:
- 型号对比:「1号和2号有什么区别」及变体(一号二号/1和2/什么不同)20+条 -> 补 什么区别/啥区别/有什么不同/怎么选/哪款/那款/1号/2号
- 体重报数:「120斤」「145斤」「90斤」20+条 -> 原配「体重」没用,观众只说「斤」,补「斤」
- 运费险:「有运费险吗」x19 -> 发货售后补 运费险/包邮/无理由/保障
- 已下单:「已拍」x28 -> 链接下单补 已拍/买了/加急
- 正品质保:「是正品吗」「质保多久」-> 质量效果补 正品/质保/保修/拆洗/能洗吗
- **新增3类**:颜色款式(看看红色/过一下颜色/没有咖色,163命中)、赠品配件(坐垫套/座套/礼盒/赠品,81命中)、使用场景(开车可以用吗/孕妇/久坐,97命中)——原来完全没有,都是真实高频

故意收紧(精度换召回):去掉裸「多少」(误伤「多少斤」)、「还有」改「还有吗」(「还有什么颜色」该归颜色款式)。价格优惠103->83、库存补货22->10 是这个原因,不是损失。

改后 7类77词 -> 10类188词。同步更新 store.DEFAULT_VOC_CATS 种子(新部署/exe 也用优化版)与实时库 voc_config。

顺带净化:发现4条商家机器人话术没被屏蔽词抓到(屏蔽词存的是「150斤以下拍加大版…卡码拍大~」,实际刷的是「150 斤…卡码往大拍~」,空格和措辞差异导致漏网)。确认7条候选全是纯机器人/客服模板(合计约2800条)、不误伤真实观众后加进屏蔽词。session 6 的VOC随之净化:尺码型号792->500、发货售后534->298、使用场景278->63、赠品配件70->43,数字终于反映真实观众而非刷屏灌水。

20 tests passed。

## 2026-08-21 17:30

- Windows入口改为无窗口启动器，后台程序更名为`douyin-dashboard.backend.exe`，启动后不再长期显示命令行黑框。
- 看板顶部新增「停止采集」，可关闭当前目录中的看板、登录辅助服务及采集子进程；保留`停止看板.vbs`作为备用入口。
- 商品讲解未登录状态新增一键登录流程，登录成功后自动保存本地状态并恢复商品采集，不再要求手动双击登录脚本。
- 房间号旁新增直播间账号名显示；热词统计和界面同时过滤纯Emoji、纯符号及表情包占位文本。
- 新增Windows发布脚本、单文件安装器和便携ZIP打包脚本；发布包排除数据库、登录状态和备份文件。
- 源码测试结果为`21 passed`；发布包在干净目录验证启动、登录辅助服务和停止流程均正常。
