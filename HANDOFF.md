...
> 给下一个会话/AI 接手用。看完这份 + `README.md` + `WORKLOG.md` 就能继续。
> 最后更新:2026-07-12。当前状态:**全部完成并交付第一版**,双数据源 + 看板 + 打包都通了。
...
- 存储:SQLite
- 看板:分组指标卡(带速率/近5分)、在线趋势(时间切换+峰值)、VOC 问题弹幕分类队列(点看原声+时间筛选)、发言榜/屏蔽池(标签切换、点用户看历史)、热词榜(点词看每次出现)、实时弹幕流(悬停屏蔽)、用户屏蔽+屏蔽词、运行时切直播间、用户等级 Lv、独立用户/累计场观
- 打包:PyInstaller onedir,内置 chromium + douyinLive,双击即用
- 双数据源:见下
...
- 优化包体积(删冗余 chromium)
- 备源也解析 sec_uid/等级(目前只有主源有,备源 user_id 仍是 111111)
- 导出复盘、健康评分模块(之前用户选"以后再说")
