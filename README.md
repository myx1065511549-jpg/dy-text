...
- 双击启动后后台运行，不再长期挂着命令行黑框。
- 看板顶部提供「停止采集」，页面内即可关闭后台服务。
- 房间号旁显示直播间账号名。
- 商品讲解未登录时可在看板内一键打开专用登录窗口，成功后自动保存并恢复商品采集。
- 热词榜同时在统计层和界面层过滤纯 Emoji、纯符号及 `[比心]` 一类表情包占位文本。

Windows 发布包可用 `build-windows-release.ps1` 生成，产物写入 `release`。发布包不会包含本地数据库、登录状态或备份文件。
